...
<div class="pd-20">
	<form id="arForm" action="<%=request.getContextPath()%>/article/expertArticleList" method="post">
	<div class="text-l">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		<span class="l">
			<a href="javascript:;" onclick="check_article('审核资讯','<%=request.getContextPath() %>/article/toCheckExpertArticlePage','800','500')" class="btn btn-primary radius">
			...
			<c:if test="${fn:length(data) gt 0}">
				<c:forEach var="content" items="${data}" varStatus="state">
					<tr class="text-c">
						...
						<td class="text-l">${content.suggestion}</td>
						<td>
...
$.jqPaginator('#pagination', {
    ...
    onPageChange: function (num) {
    	if(num!=${pager.cpage})
        	window.location.href="<%=request.getContextPath()%>/article/expertArticleList?page="+num+"&size=${pager.pageSize}&" +  + $("#arForm").serialize();
    }
});

/*审核发布（专家观点）资讯*/
function check_article(title,url,w,h) {
	...
	if($('[flag]',$(selectedBox).parent().parent()).attr('flag') == 1) {
		layer.msg("该记录已通过审核并已发布！",{icon:1,time:2000});
		return;
	}
...
function modifyVisibility() {
	...
	layer.confirm("确认隐藏/显示选择记录?",function() {
		$.ajax({
			type:"post",
            url:WEB_PATH + "/article/modifyExpertArticleVisibility",
            dataType:'json',
...
/*删除资讯*/
 function delete_article() {
	...
	layer.confirm("确认要删除吗？",function(index){
		$.ajax({
			type:"post",
			url:WEB_PATH + "/article/deleteExpertArticle",
			dataType:"json",
			data:$("#arForm").serialize(),
			success:function(data) {
				layer.msg(data.msg,{icon:1,time:2000});
...
function retriveComment(w, h) {
	...
	var url = "<%=request.getContextPath() %>/article/expertArticleCommentListByAid/" + selectedBox.val();
	var index = layer.open({
		type:2,
		area:[w + 'px', h + 'px'],
		maxmin:true,
