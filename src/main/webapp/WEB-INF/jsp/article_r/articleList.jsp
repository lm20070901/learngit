...
<div class="pd-20">
	<form id="arForm" action="<%=request.getContextPath()%>/article/researchArticleList" method="post">
	<div class="text-l">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		<span class="l">
			<a href="javascript:;" onclick="add_article('新增资讯','<%=request.getContextPath()%>/article/toCreateResearchArticlePage','800','500')" class="btn btn-primary radius">
			<i class="Hui-iconfont">&#xe600;</i> 创建
			</a> 
			<a href="javascript:;" onclick="edit_article('编辑资讯','<%=request.getContextPath()%>/article/toEditResearchArticlePage','800','500')" class="btn btn-primary radius">
...
$.jqPaginator('#pagination', {
    ...
    onPageChange: function (num) {
    	if(num!=${pager.cpage})
        	window.location.href="<%=request.getContextPath()%>/article/researchArticleList?page="+num+"&size=${pager.pageSize}&" +  + $("#arForm").serialize();
    }
});
/*
	参数解释：
...
function modifyVisibility() {
	...
	layer.confirm("确认隐藏/显示选择记录?",function() {
		$.ajax({
			type:"post",
            url:WEB_PATH + "/article/modifyResearchArticleVisibility",
            dataType:'json',
...
function publishArticle() {
	...
	layer.confirm("是否审核发布?",function() {
		$.ajax({
			type:"post",
			url:WEB_PATH + "/article/publishResearchArticle",
			dataType:"json",
			data:$("#arForm").serialize(),
			success:function(data) {
				layer.msg(data.msg,{icon:1,time:2000});
...
/*删除资讯*/
 function delete_article() {
	...
	layer.confirm("确认要删除吗？",function(index){
		$.ajax({
			type:"post",
			url:WEB_PATH + "/article/deleteResearchArticle",
			dataType:"json",
			data:$("#arForm").serialize(),
			success:function(data) {
				layer.msg(data.msg,{icon:1,time:2000});
...
function retriveComment(w, h) {
	...
	var url = "<%=request.getContextPath() %>/article/researchArticleCommentListByAid/" + selectedBox.val();
	var index = layer.open({
		type:2,
		area:[w + 'px', h + 'px'],
		maxmin:true,
